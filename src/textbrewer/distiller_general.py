...
class GeneralDistiller(BasicDistiller):
    ...
    def __init__(self, train_config,
        ...
        super().__init__(train_config, distill_config, model_T, model_S, adaptor_T, adaptor_S)
    ...
    def save_and_callback(self, global_step, step, epoch, callback):
        ...
        super().save_and_callback(global_step, step, epoch, callback)

        if self.has_custom_matches:
            self.model_S._forward_hooks = handles_S  # restore hooks
            self.model_T._forward_hooks = handles_T

    def train(self, optimizer, dataloader, num_epochs, scheduler_class=None, scheduler_args=None, scheduler=None,
        ...
        return super().train(optimizer, dataloader, num_epochs, scheduler_class, scheduler_args, scheduler, max_grad_norm,
                      num_steps, callback, batch_postprocessor, **args)
        ...
        if 'logits' in results_T and 'logits' in results_S:
            ...
            if self.d_config.probability_shift is True:
                ...
                for l_T, l_S in zip(logits_list_T, logits_list_S):
                    ...
                    kd_loss = self.kd_loss(l_S, l_T, temperature) * self.d_config.kd_loss_weight
                    total_loss += kd_loss
                print(f'total_kd_loss: {total_loss.item()}')
